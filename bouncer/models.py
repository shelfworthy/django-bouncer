...
class Invite(models.Model):
    ...
    @property
    def token(self):
        return hashlib.sha1(settings.SECRET_KEY + unicode(int_to_base36(self.id)) + unicode(self.is_accepted) + unicode(self.is_ignored) + self.date_added.strftime('%Y-%m-%d %H:%M:%S')).hexdigest()[::2]
